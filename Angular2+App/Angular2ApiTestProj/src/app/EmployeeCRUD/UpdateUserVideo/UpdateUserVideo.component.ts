...
@Component({
  selector: 'app-UpdateUserVideo',
  templateUrl: './UpdateUserVideo.component.html',
  styleUrls: ['./UpdateUserVideo.component.css']
})
export class UpdateUserVideoComponent implements OnInit {
  pageTitle = "User List_Edit User Video"
  ...
  changeImage(e: any, inputFile: File | null) {
    ...
    // this.serviceUserApiService.uploadVideo(id, file).subscribe(res => {
    this.serviceUserApiService.uploadVideo(id, file).subscribe({
      next: (res) => {
        console.log(res);
